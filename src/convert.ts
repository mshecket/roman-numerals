/**
 * This function is based on the modern standard form
 * for Roman numerals, as expressed here:
 *
...
function convert (input: any): any {
  if (
    typeof input === 'string' &&
    // This regex tests for the presence of a valid permutation for each decimal place.
    /^M*(C{0,3}|CD|DC{0,3}|CM)?(X{0,3}|XL|LX{0,3}|XC)?(I{0,3}|IV|VI{0,3}|IX)?$/i.test(
      input
    )
  ) {
